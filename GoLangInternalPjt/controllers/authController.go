...
func SendResetEmail(toEmail string, resetToken string) error {
    ...
    resetLink := fmt.Sprintf("http://localhost:8080/auth/reset-password-link?token=%s", resetToken)
...
func RedirectToResetPage(c *gin.Context) {
	token := c.Query("token")
	frontendURL := fmt.Sprintf("http://localhost:3000/reset-password?token=%s", token)
